...
struct SpirvShaderParameters {
    ...
    // dynamically-computed buffer index so a garbage value cannot read a stale/out-of-range
    // slot (which yields a wild device address and a GPU page fault / device loss).
    int buffer_count = 0;

    // SECATTR register numbers that hold encoded (index<<28) uniform-buffer handles. A load whose
    // base is one of these is a normal uniform-buffer access (encoded fast path); any other base is
    // a junk computed "pointer" that is redirected to a safe buffer instead of being decoded.
